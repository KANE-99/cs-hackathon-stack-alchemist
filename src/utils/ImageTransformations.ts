  ...
  if (options) {
    ...
    if (options.overlay) {
      ...
      if (options.overlay?.["overlay-align"]) {
        ...
        const align = `${alignColumn[alignPosition[0]]},${
          alignRow[alignPosition[1]]
        }`;
        queryParams += `&overlay-align=${align}`;
      }
      if (
        options.overlay?.["overlay-repeat"] &&
        options.overlay?.["overlay-repeat"] !== "none"
      ) {
...
export const getImageStyles = (options) => {
  let styles = {};
  if (options?.transform?.rotate) {
    styles["transform"] = `scale(${
      1 + Math.abs(Math.ceil((options?.transform?.rotate || 0) / 10)) / 10
    }) rotate(${options?.transform?.rotate || 0}deg)`;
  }
  return styles;
};
const getEffectsParams = (effects) => {
  ...
  if (effects.sharpen) {
    params += `&sharpen=a${effects.sharpen?.amount || 0},r${
      effects.sharpen?.radius || 1
    },t${effects.sharpen?.threshold || 0}`;
  }
  return params;
};
 ...
 *
 */
export const resolvePresetByPresetName = ({
  asset,
  presetName,
  extension_uid,
}) => {
 ...
 */
export const fetchPresetByPresetName = ({
  asset,
  extension_uid,
  presetName,
}) => {
 ...
 */
export const resolvePresetStylesByPresetName = ({
  asset,
  presetName,
  extension_uid,
}) => {
 ...
 */
export const resolvePresetByPresetUID = ({
  asset,
  presetUID,
  extension_uid,
}) => {
 ...
 */
export const resolvePresetStylesByPresetUID = ({
  asset,
  presetUID,
  extension_uid,
}) => {
